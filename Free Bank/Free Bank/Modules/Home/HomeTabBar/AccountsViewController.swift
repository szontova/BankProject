...
class AccountsViewController: UIViewController {
    ...
    func updateAccounts() {
        ...
        simpleAccounts = accounts.filter({ acc in
            return MyCustomVC.getAccCategory(acc) == "S"
        })
        depositAccounts = accounts.filter({ (acc) -> Bool in
            return MyCustomVC.getAccCategory(acc) == "D"
        })
...
extension AccountsViewController: UITableViewDataSource {
    func setCountOfSection() -> Int {
        if creditAccounts.isEmpty && depositAccounts.isEmpty { return 1 } else if creditAccounts.isEmpty || depositAccounts.isEmpty { return 2 } else { return 3 }
    }
    func getAccount (row: Int, section: Int) -> Account {
        var acc: Account = accounts[0]
        if setCountOfSection() == 2 {
            if depositAccounts.count != 0 {
                if section == 0 { acc = depositAccounts[row] } else { acc = simpleAccounts[row] }
            } else {
                if section == 0 { acc = creditAccounts[row] } else { acc = simpleAccounts[row] }
            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = self.accountsTableView.dequeueReusableCell(withIdentifier: AccountTableViewCell.identifier) as? AccountTableViewCell {
            ...
            var count = 0
            if MyCustomVC.getAccCategory(acc) == "S" { count = simpleAccounts.count}
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if MyCustomVC.getAccCategory(accountForTransfer!) != "D" {
